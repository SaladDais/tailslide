...
TEST_CASE("Simple register initialization") {
  ...
  LSOBitStream script_bs(std::move(visitor.script_bs));

  script_bs.move_to(LSORegisterOffsets[LREG_VN]);
...
TEST_CASE("Simple heap compilation") {
  ...
  auto *list_const = parser.allocator.new_tracked<LSLListConstant>(nullptr);
  list_const->push_child(parser.allocator.new_tracked<LSLIntegerConstant>(1));
  auto *vec_child = parser.allocator.new_tracked<LSLVectorConstant>(1, 2, 3);
  list_const->push_child(vec_child);
  LSOHeapManager heap_manager;
  heap_manager.writeConstant(list_const);

  LSOBitStream heap_bs(std::move(heap_manager.heap_bs));
  ...
  heap_bs.move_to(vec_pos);
  Vector3 v;
  heap_bs >> v;
  // make sure the vector matches what we meant to write
  CHECK_EQ(*vec_child->get_value(), v);
